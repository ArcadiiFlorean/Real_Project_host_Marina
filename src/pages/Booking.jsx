...
function Booking() {
  ...
  const fetchOrderAndSlots = async () => {
    try {
      // Fetch order
      ...
      if (orderData.status !== 'completed') {
        alert('Plata nu a fost confirmată încă. Verifică statusul comenzii.');
        navigate('/');
        return;
      }
      ...
      // Fetch available slots (viitoare și nedisponibile)
  ...
  const handleBooking = async () => {
    if (!selectedSlot) {
      alert('⚠️ Selectează o dată și oră!');
      return;
    }

    if (!clientInfo.name || !clientInfo.email) {
      alert('⚠️ Completează numele și email-ul!');
      return;
    }
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        >
          ...
          <p className="text-gray-600 text-lg">
            Acum alege data și ora pentru consultația ta
          </p>
        </motion.div>
        ...
        >
          <h3 className="font-bold text-gray-800 mb-2">Pachet Achiziționat:</h3>
          <p className="text-2xl font-bold bg-gradient-to-r from-pink-600 to-orange-600 bg-clip-text text-transparent">
            {order?.package_name}
          </p>
          <p className="text-gray-600 mt-2">Valoare: £{order?.amount}</p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-2xl font-bold text-gray-800">📅 Alege Data & Ora</h3>
            </div>

            {Object.keys(groupedSlots).length === 0 ? (
              <div className="text-center py-12">
                <div className="text-6xl mb-4">📅</div>
                <p className="text-gray-500">Nu există slot-uri disponibile momentan</p>
                <p className="text-gray-400 text-sm mt-2">Te rugăm să contactezi Marina direct</p>
              </div>
            ) : (
                  ...
                  <div key={date} className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {slots.map((slot) => (
                        ...
                        >
                          🕐 {new Date(slot.start_time).toLocaleTimeString('ro-RO', { 
                            hour: '2-digit', 
                            minute: '2-digit' 
                          })}
                        </motion.button>
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-2xl font-bold text-gray-800">📝 Datele Tale</h3>
            </div>
            ...
            <div className="space-y-4">
              ...
              {selectedSlot && (
                ...
                >
                  <p className="text-sm font-medium text-gray-700 mb-1">Slot selectat:</p>
