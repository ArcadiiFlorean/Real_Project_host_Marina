...
function AdminDashboard({ user, onLogout }) {
  ...
  const handleAutoTranslate = async () => {
    ...
    try {
      ...
      const featuresEn = await Promise.all(
        formData.features_ro.filter(f => f.trim() !== '').map(feature => translateText(feature, 'en'))
      );
      ...
      const featuresRu = await Promise.all(
        formData.features_ro.filter(f => f.trim() !== '').map(feature => translateText(feature, 'ru'))
      );
  ...
  const handleSubmit = async (e) => {
    ...
    const finalData = {
      ...
      features_en: formData.features_en.filter(f => f.trim() !== '').length > 0 
        ? formData.features_en.filter(f => f.trim() !== '')
        : formData.features_ro.filter(f => f.trim() !== ''),
      features_ru: formData.features_ru.filter(f => f.trim() !== '').length > 0
  ...
  const removeFeature = (lang, index) => {
    const newFeatures = formData[`features_${lang}`].filter((_, i) => i !== index);
    setFormData({
      ...formData,
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-6 py-8">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8 mb-8 animate-fade-in">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    />
                    <p className="text-xs text-blue-600 mt-1">💡 Se va traduce automat</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      🇷🇺 Русский (RU)
                ...
                {formData.features_ro.map((feature, index) => (
                  <div key={index} className="flex gap-2 mb-2">
                    <input
                      type="text"
                      value={feature}
                      onChange={(e) => updateFeature('ro', index, e.target.value)}
              ...
              <div>
                ...
                <p className="text-xs text-blue-600 mb-2">💡 Se va completa automat la traducere</p>
                ...
                {formData.features_ru.map((feature, index) => (
                  <div key={index} className="flex gap-2 mb-2">
                    <input
                      type="text"
                      value={feature}
                      onChange={(e) => updateFeature('ru', index, e.target.value)}
                      className="flex-1 px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 bg-blue-50"
                      placeholder="Se va traduce automat"
                    />
                    <button
              ...
              <div className="flex gap-4 pt-6">
                ...
                >
                  {loading ? 'Se salvează...' : editingPackage ? '💾 Salvează' : '➕ Adaugă Pachet'}
                </button>
                <button
                  type="button"
                  onClick={resetForm}
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <h2 className="text-2xl font-bold text-gray-800 mb-6">💝 Pachete de Consultație</h2>
          ...
          {loading ? (
            ...
            <p className="text-gray-500 text-center py-12">Nu există pachete. Adaugă primul pachet!</p>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {packages.map((pkg) => (
                <div key={pkg.id} className="border rounded-xl p-6 hover:shadow-lg transition-all relative">
                  {pkg.is_popular && (
                  ...
                  <h3 className="text-xl font-bold text-gray-800 mb-2">{pkg.name_ro}</h3>
                  <p className="text-sm text-gray-600 mb-4">{pkg.description_ro}</p>
                  ...
                  <div className="flex items-center justify-between mb-4 text-sm text-gray-600">
                    <span className="font-bold text-pink-600 text-lg">{pkg.price} €</span>
